"""
Homework - 5
Desc: This program is used to calculate and store different 
	  relevance factor, such as recall, precision, etc.
"""
__author__ = "Anuj Thakur"
...
def read_all_documents(path):
    """ 
	Desc: Gets all ID and Score for particular query from results of HW4
    """
    with open(path, 'r') as file_obj:
            for line in file_obj.readlines():
                temp_list = line.split(' ')
                rank = int(temp_list[0])
                id = temp_list[1]
                score = float(temp_list[2])
...
def read_relevence(query_id):
    """
    @param query_id
	Desc: Get relevant documents w.r.t. query ID
    """
    with open('relevance.txt','r') as file_obj:
        for line in file_obj.readlines():
            temp_list = line.split(' ')
            if temp_list[0] == str(query_id):
                relevant_docs[query_id].append(int(temp_list[2][temp_list[2].find('-')+1:]))

def calculate_precision_and_recall(query_id):
    """
    @param query_id: Query ID
    Desc: Gets precision and recall for all documents from then result of HW4
    """
...
def mean_average_precision(input_query_id):
    """ Desc: Calculate precision for all documents given ion the input_query_id"""
    sum = 0
...
def get_actual_dcg():
    """ Desc: Calculated the actual DCG """
    dcg = 0
...
def get_ndcg():
    """ Desc: Calculates NDCG and stores it in the master dictionary named document_dict """
...
def reintialize():
    """ Desc: Reinitialize entities before switching to the next query"""
...
if __name__ == "__main__":
    file_names = ['portable_operating_systems.txt', 'code_optimization_for_space_efficiency.txt', 'parallel_algorithms.txt']
    ids = [12, 13,19]
    for id, name in zip(ids, file_names):
        print "Query " + name + "(" + str(id) + ")"
        print "RANK ID SCORE RELEVANCE_LEVEL PRECISION RECALL NDCG"
        reintialize()
        read_relevence(id)
        read_all_documents(name)
        calculate_precision_and_recall(id)
        get_actual_dcg()
        get_ideal_dcg(id)
        get_ndcg()
